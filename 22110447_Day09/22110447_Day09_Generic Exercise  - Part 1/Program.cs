...
namespace _22110447_Day09_Generic_Exercise____Part_1 {
    internal class Program {
        static void Main(string[] args) {
            ...
            foreach(var item in firstList) {
                Console.WriteLine($"{item.MovieName} {item.Director} {item.Rate} {item.ReleaseDate.ToShortDateString()}");
            }

            foreach(var item in secondList) {
                Console.WriteLine($"{item.MovieName} {item.Director} {item.Rate} {item.ReleaseDate}");
            }
        }
    }
    class Movies<T, U> {
